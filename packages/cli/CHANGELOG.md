...
- 33ab5f7: Every live schema version of the public read contract now gets its own route prefix — `/api/v1/blog` and `/api/v3/blog` serve the same database row under whatever field names that version's schema declared, so a consumer can pin a version and keep receiving its exact shape after later versions are cut.
