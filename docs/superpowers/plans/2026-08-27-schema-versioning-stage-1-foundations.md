...
Stage 2 (versioning proper) plugs into the two boundaries this stage created: `FieldKeyMap` generalizes into a per-version `VersionProjection`, `paths.ts` gains the version segment, and core begins producing divergent `column_name` values by folding the rename chain. Nothing in Stage 2 needs to revisit the call sites audited here.
