...
completely fresh `node_modules` (as in every Docker build) it treats
unapproved build scripts as an error. The Docker build used `corepack enable`
without a pinned version, potentially downloading a newer pnpm that is stricter.
...
This restores the pre-v10 behaviour where `pnpm deploy` symlinks workspace
...
| `infra/lib/fargate-stack.ts` | Health check path changed from `/` to `/api/openapi.json` |
...
curl -s -o /dev/null -w "%{http_code}" \
  http://<alb-dns>/api/openapi.json

# Should return UNAUTHORIZED JSON (server is running, auth is working)
curl -s http://<alb-dns>/admin/api/config
```
